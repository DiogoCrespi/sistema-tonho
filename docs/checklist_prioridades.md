...
| **SEC-06** | CSRF Segregado | 9/10 | **Implementado** | 6h | Médio | Backend |
| **SEC-07** | Rate Limit IP + Conta | 8/10 | **Implementado** | 4h | Baixo | Backend |
...
| **DB-01** | Persistência (Docker Volumes) | 10/10 | **Implementado** | 4h | Baixo | DevOps |
...
| **UX-05** | Cache Nginx e Cache-Busting | 7/10 | **Não Iniciado** | 8h | Baixo | DevOps |
...
|---|---|---|---|
| **SEC-01** | `../backend/src/index.js` | - | `Verificar IDOR negativo cruzado entre personais` |
| **SEC-06** | `../backend/src/middleware/httpSecurity.js` | `../backend/tests/integration/security.test.js` | - |
| **DB-01** | `../docker-compose.yml` | - | `Checagem física de persistência pós container restart` |
| **DB-02** | `../backend/knexfile.js` | - | `Testes de estresse com concorrência paralela` |
| **DB-03** | `../backend/knexfile.js` | - | `Tentar inserir registro órfão e verificar falha` |
| **DB-04** | `../backend/src/scripts/backupDatabase.js` | `../backend/tests/integration/backup.test.js` | - |
| **OPS-10** | `../backend/src/index.js` | - | `Separar liveness de readiness verificando migrations` |
...
*   *Critério 1*: Qualquer requisição a `/api/workouts/:id` ou `/api/measurements/:id` de um aluno associado a outro Personal deve retornar HTTP `403 Forbidden`.
*   *Critério 2*: A rejeição de acesso IDOR deve registrar um log na tabela `audit_logs` registrando o IP e a conta tentativa.
...
*   *Critério 2*: Queries de delete em alunos que tenham medições ou treinos ativos devem falhar se a cascade referencial não estiver explícita.

...
*   **Especificação**: Ajustar o arquivo `docker-compose.yml` para declarar named volumes para SQLite `/app/data` e uploads `/usr/src/app/backend/uploads`.
...
*   **Especificação**: Refatorar script em `backend/src/scripts/backupDatabase.js` para usar a instrução do SQLite `VACUUM INTO 'backups/db.sqlite'` na criação de snapshots atômicos, evitando cópia de arquivos temporários desvinculados.
...
*   **Especificação**: O Express converte imagens Base64 recebidas para WebP, salvando o arquivo físico na pasta uploads. Em caso de exclusão lógica ou física do banco, acionar a remoção física no disco via `fs.promises.unlink`.

### [UX-05] Controle de Cache no Nginx e Cache-Busting
*   **Especificação**: Headers `Cache-Control: no-cache` no Nginx para `index.html`. Hashing exclusivo nos nomes dos bundles JavaScript e CSS gerados no build.
